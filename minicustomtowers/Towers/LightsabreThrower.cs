...
namespace minicustomtowers.Towers
{
    class LightsabreThrower
    {
        ...
        { 
                ...
                {
                    LocalizationManager.instance.textTable.Add(customTowerName, "Lightsabre Thrower");
                }
            ...
            CacheBuilder.toBuild.PushAll("LightsabreThrower", "LightSabre", "LightsabreMonkey");
            }
        ...
        static string customTowerName = "Lightsabre Thrower";
        ...
        {
            TowerModel towerModel = gameModel.GetTowerFromId("BoomerangMonkey-002").Duplicate<TowerModel>(); //gameModel.GetTowerFromId(Alchemist).Duplicate<TowerModel>();
            towerModel.name = customTowerName;
            towerModel.baseId = customTowerName;
            towerModel.display = "LightsabreMonkey";
            towerModel.GetBehavior<DisplayModel>().display = "LightsabreMonkey";
            towerModel.GetBehavior<DisplayModel>().layer = 109;
            towerModel.portrait = new SpriteReference(guid: "LightsabreThrower");
            towerModel.icon = new SpriteReference(guid: "LightsabreThrower");
            towerModel.instaIcon = new SpriteReference(guid: "LightsabreThrower");
            towerModel.towerSet = "Primary";
            towerModel.dontDisplayUpgrades = true;
            towerModel.cost = 1850f;
            towerModel.isGlobalRange = false;
            towerModel.tiers = new int[] { 0, 0, 0 };
            var attackModel = towerModel.GetBehavior<AttackModel>();
            attackModel.weapons[0].projectile.pierce = 70f;
            attackModel.weapons[0].projectile.display = "LightSabre";

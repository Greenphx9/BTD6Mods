...
namespace minicustomtowers.Towers
{
    class CannonDestroyer
    {
        ...
        { 
                ...
                {
                    LocalizationManager.instance.textTable.Add(customTowerName, "Cannon Destroyer");
                }
            ...
            CacheBuilder.toBuild.PushAll("CannonDestroyer");
            }
        ...
        static string customTowerName = "Cannon Destroyer";
        ...
        {
            TowerModel towerModel = gameModel.GetTowerFromId("MonkeyBuccaneer-320").Duplicate<TowerModel>(); //gameModel.GetTowerFromId(Alchemist).Duplicate<TowerModel>();
            towerModel.name = customTowerName;
            towerModel.baseId = customTowerName;
            towerModel.portrait = new SpriteReference(guid: "CannonDestroyer");
            towerModel.icon = new SpriteReference(guid: "CannonDestroyer");
            towerModel.instaIcon = new SpriteReference(guid: "CannonDestroyer");
            towerModel.dontDisplayUpgrades = true;
            towerModel.cost = 5410f;
            towerModel.isGlobalRange = false;
            towerModel.tiers = new int[] { 0, 0, 0 };
            var attackModel = towerModel.GetBehavior<AttackModel>();
            var bombProj = gameModel.GetTowerFromId("MonkeyBuccaneer-230").GetWeapon(4).projectile.Duplicate<ProjectileModel>();
            attackModel.weapons[0].projectile = bombProj;
            attackModel.weapons[1].projectile = bombProj;
            return towerModel;
        }
